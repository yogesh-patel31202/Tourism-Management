<?php
require_once(__DIR__ . '/../service/HotelService.php');
...
class HotelAction {
    ...
    private function processPostRequest() {
        ...
        if (isset($_FILES["Image"])) {
            ...
            $uploadFile = $uploadDir . "hotel_" . $max_id_value . ".jpg";
           ...
           // imagecreatefromjpeg($inputFile);
            if (in_array($imageFileType, $allowedExtensions)) {
                ...
                if($dto->getId()>0){
                    $filePath = $uploadDir . "hotel_" . $dto->getId() . ".jpg";
        ...
        if ($dto->getId() == 0) {
            $b = $ser->insertHotel($dto);

            if ($b) {
                header("Location: ../../add_hotel.php?msg=Yes");
                exit();
            } else {
                header("Location: ../../add_hotel.php?msg=No");
                exit();
            }
        } else {
            $b = $ser->updateHotel($dto);
